...
# ТЕСТОВАЯ ПОДПИСКА ДОБАВЛЕНИЕ
@dp.message_handler(state=FormStatesTestSubs.NAME_SUBS)
async def process_device_type(message: types.Message, state: FSMContext):
    device_name = message.text
    response = await api_client.add_device(message.from_user.id, 'test rate', device_name, datetime.now().strftime("%Y-%m-%dT%H:%M:%S"))
...
async def process_device_type(message: types.Message, state: FSMContext):
    ...
    if promo == key:
        ...
        await message.answer("🔤 Введите название для новой подписки")
        await FormStatesTestSubs.next()
    else:
